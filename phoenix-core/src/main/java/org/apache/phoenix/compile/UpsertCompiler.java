...
public class UpsertCompiler {
    ...
    public MutationPlan compile(UpsertStatement upsert) throws SQLException {
        ...
        //add by dongping 20181031 begin
        nodeIndex = 0;
        List<String> expressionList = new ArrayList<>(constantExpressions.size());
        for (Expression expression : constantExpressions) {
            PColumn column = allColumns.get(columnIndexes[nodeIndex]);
            String expressionValue = "";
            if (expression instanceof LiteralExpression){
                expressionValue = ((LiteralExpression)expression).getValue() + "";
            }
            expressionList.add(column.getName() + ": " + expressionValue + " ");
            nodeIndex++;
        }
